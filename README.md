<div align="center">

# 🚀 Daniel Diniz  
### Software Engineering Student | Full Stack Developer & Data Analyst  

<p align="center">
  <img src="https://img.shields.io/badge/React-20232A?style=for-the-badge&logo=react&logoColor=61DAFB" />
  <img src="https://img.shields.io/badge/Next.js-000000?style=for-the-badge&logo=nextdotjs&logoColor=white" />
  <img src="https://img.shields.io/badge/TypeScript-007ACC?style=for-the-badge&logo=typescript&logoColor=white" />
  <img src="https://img.shields.io/badge/Tailwind_CSS-38B2AC?style=for-the-badge&logo=tailwind-css&logoColor=white" />
  <img src="https://img.shields.io/badge/Python-3776AB?style=for-the-badge&logo=python&logoColor=white" />
  <img src="https://img.shields.io/badge/Java-%23ED8B00.svg?style=for-the-badge&logo=openjdk&logoColor=white" />
  <img src="https://img.shields.io/badge/Supabase-3FCF8E?style=for-the-badge&logo=supabase&logoColor=white" />
  <img src="https://img.shields.io/badge/PostgreSQL-316192?style=for-the-badge&logo=postgresql&logoColor=white" />
</p>

> **Portfolio Técnico 2026**  
> Repositório dedicado a demonstrar minha evolução técnica, acadêmica e profissional durante a graduação em  
> **Análise e Desenvolvimento de Sistemas (2025–2027)**.

</div>

---

## 👨‍💻 Sobre Mim

Sou um desenvolvedor em **transição de carreira**, com forte base em **Análise de Dados** e foco em **Engenharia de Software**.

Atualmente construo **produtos reais em produção**, aplicando boas práticas de arquitetura, performance e experiência do usuário.

- 🎓 **Formação:** Análise e Desenvolvimento de Sistemas (2025 – 2027)  
- ⚡ **Foco:** Arquitetura de Sistemas, Clean Code, Performance  
- 🛠️ **Diferencial:** Forte background em BI (Power BI / Excel)  
- 🚀 **Objetivo:** Estágio / Trainee em Tecnologia  

---

## 💻 Stack Tecnológica

| Camada        | Tecnologias                                                                 |
|---------------|------------------------------------------------------------------------------|
| **Frontend**  | React, Next.js 15, TypeScript, Tailwind CSS, JavaScript (ES6+)               |
| **Backend**   | Python, Java, APIs REST, Supabase (PostgreSQL)                               |
| **Cloud**     | Vercel, Render, AWS (em estudo)                                              |
| **Data & BI** | Power BI, Power Query, ETL                                                   |
| **Ferramentas** | Git, GitHub, Jira, Figma                                                   |

---

## 💎 Projetos em Destaque

### 🩺 TI-Saúde — Plataforma Web  
🔗 https://ti-saude.vercel.app  

Plataforma voltada ao nicho de TI e saúde, com foco em performance, SEO e experiência do usuário.

**Stack:**  
Next.js 15 • TypeScript • Framer Motion • Vercel  

---

### 🤖 Sistema de Vendas com IA  
🔗 https://meu-sistema-vendas.vercel.app  

Ferramenta para geração de páginas e automação de vendas utilizando Inteligência Artificial.

**Stack:**  
Next.js 15 (App Router) • Supabase (PostgreSQL) • OpenAI (GPT-4o-mini) • Pollinations AI  

---

### 🏢 Protótipo Actus — Site Corporativo  
🔗 https://prototipo-actus.vercel.app  

Protótipo de site corporativo com foco em SEO, semântica HTML5 e layout moderno.

**Stack:**  
HTML5 • CSS Grid & Flexbox • SEO  

---

## 📈 Diferenciais Profissionais

- Experiência com **produtos em produção**  
- Integração de **IA em aplicações web**  
- Forte base em **análise de dados e BI**  
- Escrita de código limpo e escalável  
- Documentação técnica clara (PT / EN)  

---

## 🤝 Conecte-se Comigo

<p align="left">
  <a href="https://www.linkedin.com/in/daniel-diniz-186b52339">
    <img src="https://img.shields.io/badge/LinkedIn-0077B5?style=flat-square&logo=linkedin&logoColor=white" />
  </a>
  <a href="mailto:dinizdaniel284@gmail.com">
    <img src="https://img.shields.io/badge/Email-D14836?style=flat-square&logo=gmail&logoColor=white" />
  </a>
  <a href="https://github.com/dinizdaniel284">
    <img src="https://img.shields.io/badge/GitHub-000000?style=flat-square&logo=github&logoColor=white" />
  </a>
</p>

---

<div align="center">
  <i>"A disciplina é a ponte entre metas e realizações."</i><br>
  <b>Análise e Desenvolvimento de Sistemas — 2025–2027</b>
</div>
